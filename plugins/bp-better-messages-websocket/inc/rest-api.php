...
public function delete_messages( WP_REST_Request $request ) {
    ...
    foreach ($messages_ids as $message_id) {
        ...
        // Fetch thread ID and group ID associated with this message
        $thread_id = $message->thread_id;
         $group_id = Better_Messages()->functions->get_thread_meta($thread_id, 'um_group_id');
        // Fetch 'enable_feature' and group creator ID
        ...
        $canDelete = false;
        // Allow the group creator to delete all messages in the group if 'enable_feature' is 0
        if ($enable_feature == '0' && $user_id == $group_creator_id) {
            $canDelete = true;
        } else {
            $errors[] = _x('You are not allowed to delete messages in this group.', 'Rest API Error', 'bp-better-messages');
        }

        // If allowed, delete the message
        if ($canDelete) {
    ...
    $return = $replaceMethod ? Better_Messages()->api->get_messages(null, $deleted_messages) : ['deleted' => $deleted_messages];
    ...
    return $return;
        ...
        public function delete_messages_old( WP_REST_Request $request ){
...
public function can_reply(WP_REST_Request $request) {
    ...
    // Stop here if user doesn't have access
    if (! $has_access) {
        $temp_id = sanitize_text_field($request->get_param('temp_id'));
        do_action('better_messages_on_message_not_sent', $thread_id, $temp_id, []);
    ...
    // If it's a group thread, enforce enable_feature logic
    if ($type === 'group') {
        // Retrieve the group ID associated with the thread
        $group_id = Better_Messages()->functions->get_thread_meta($thread_id, 'um_group_id');
        error_log("group_id in rest api: " . $group_id);

        if ($group_id) {
            // Check if the group has the 'enable_feature' set to '1'
            $enable_feature = get_post_meta($group_id, '_um_groups_enable_feature', true);
            error_log("enable_feature in rest api: " . $enable_feature);

            // Retrieve the group creator ID
            $group_creator_id = get_post_field('post_author', $group_id);
            $group_title = get_post_field('post_title', $group_id);
            error_log($group_title . "creater id: " . $group_creator_id);
            error_log("curent user id" . $user_id);
            
            // If 'enable_feature' is '1', allow only the creator to reply
            if ($enable_feature === '1') {
                if ($user_id == $group_creator_id) {
                    // Allow the creator to send messages
                    return true;
                } else {
            ...
            error_log("No group_id associated with this thread.");
        }
    }

    // If not restricted by group or enable_feature, proceed with general access
    return $has_access;
}
